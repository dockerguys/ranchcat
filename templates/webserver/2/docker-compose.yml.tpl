...
{{- end }}
    ...
    # -----------------------------------
    environment:
      SYSTEM_SERVICES_GRACETIME: "${svcd_shutdown_grace}"
      NGINX_IGNORE_INVALID_HEADERS: "on"
      NGINX_ALLOW_UNDERSCORE_IN_HEADERS: "off"
...
{{- if eq .Values.nginx_console_log "true" }}
      NGINX_CONSOLE_LOG: "yes"
{{- else }}
      NGINX_CONSOLE_LOG: "no"
{{- end }}
...
{{- if eq .Values.nginx_enable_webapi_svc "true" }}
      NGINX_ENABLE_WEBAPI_PUBLIC_SERVICES: "yes"
{{- else }}
...
{{- if eq .Values.nginx_enable_subsite_svc "true" }}
      NGINX_ENABLE_SUBSITE_HOSTING: "yes"
{{- else }}
      NGINX_ENABLE_SUBSITE_HOSTING: "no"
{{- end }}
{{- if eq .Values.nginx_enable_httpsify_svc "true" }}
      NGINX_ENABLE_HTTPS_REDIRECTOR: "yes"
{{- else }}
      NGINX_ENABLE_HTTPS_REDIRECTOR: "no"
{{- end }}
{{- if eq .Values.nginx_enable_www_redirect_svc "true" }}
      NGINX_ENABLE_WWW_REDIRECTOR: "yes"
{{- else }}
      NGINX_ENABLE_WWW_REDIRECTOR: "no"
{{- end }}
    # -----------------------------------
    # Scheduler labels
    # -----------------------------------
