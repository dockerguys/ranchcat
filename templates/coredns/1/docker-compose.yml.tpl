...
{{- end }}
    ...
    environment:
      ...
      COREDNS_ENABLE_AUTOZONE: ${coredns_enable_zones}
...
{{- else }}
      ...
      COREDNS_RATELIMIT_LOG_ONLY: false
...
{{- if (.Values.coredns_ipindn_domain) }}
      COREDNS_ENABLE_IPINDN: true
      COREDNS_IPINDN_DOMAIN: ${coredns_ipindn_domain}
      COREDNS_IPINDN_FALLTHROUGH: false
{{- end }}
...
{{- if eq .Values.coredns_disable_udp "false" }}
    # -----------------------------------
    # Expose ports
    # -----------------------------------
