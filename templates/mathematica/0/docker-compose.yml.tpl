...
{{-   if (.Values.mma_image_custom) }}
    image: "${docker_registry_name}/${mma_image_custom}"
{{-   else }}
    image: "${docker_registry_name}/${mma_image}"
{{-   end }}
{{- else }}
{{-   if (.Values.mma_image_custom) }}
    image: ${mma_image_custom}
{{-   else }}
    image: ${mma_image}
{{-   end }}
{{- end }}
    ...
    # -----------------------------------
    environment:
      MATHPASS_MAGIC_INDEX: 1
      MATHPASS_ACTIVATE_KEY: "1234-4321-123456"
    # -----------------------------------
    # Scheduler labels
    # -----------------------------------
    labels:
      io.rancher.container.hostname_override: container_name
      io.mathematica.role: "{{ .Stack.Name }}/engine"
